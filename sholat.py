# sholat
# iilhaaaammm
# created 18, februari 2023 2:18 pm
# updated 23 maret 2023 6:00 pm
#-*-coding : utf-8-*-
...
def trdsholat():
      ...
      st = [lr,
      'JANGAN DI CANCELL KALO ADZANNYA BUNYI'
      ]
      for i in st:
      	print(i.center(60))
...
def banner():
      ...
      ╚══════╝╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═╝  ╚═╝   ╚═╝
              {lw}Programmer Muslim Tidak Lupa Ibadah{lg}
                  {lg}[{x}Spesial Ramadhan  2023/1444 H{lg}]
••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••
''')                                                  
...
def menu():
      banner()
      print(f'''
{lg}1.{lw} Lihat jadwal sholat
{lg}2.{lw} Ganti kota
{lg}3.{lw} Update script
{lg}4.{lw} Tentang
{lg}0.{lw} Keluar''')
...
def update():
      banner()
      print(lr+'Jangan di cancel biar ga error :*')
      print(lg+'Cek jaringan..')
      try:
            get('https://github.com')
      except requests.exceptions.ConnectionError:
            print(lg+'Hidupin internetnya')
            exit()
      print(lg+'Meng-update..\nLama tidaknya tergantung jaringan, sabarr :)')
      
      os.system('cd .. && rm -rf sholat')     
      sp.call(['cd .. && git clone https://github.com/hammyh22/jadwal-shalat'],shell=True, stdout=sp.DEVNULL,stderr=sp.STDOUT)
      print(lg+'Selesai meng-update')
      print(lg+'Memulai ulang..')
      sleep(2)
      os.system('cd ../data && python sholat.py')
def tentang():
      banner()
      print(f'''
         
{lg}Nama        : {lw}Jadwal Shalat
{lg}Versi       : {lw}1.0
{lg}Tanggal     : {lw}{tm('%d %B, %Y')}
{lg}Author      : {lw}iilhaaaammm
{lg}Tujuan      : {lw}Mengingatkan kita pada waktu sholat
{lg}Terimakasih : {lw}Allah SWT
{lg}NB          : {lw}Tidak ada manusia yang sempurna, semua manusia pasti memiliki kekuranga dan juga kelebihan masing-masing.
              Kritik & saran :
              - https://t.me/yassumii
              - https://instagram.com/696ssens''')
      input(lg+'Klik enter ')
      menu()     
...
def exit():
      print(lg+'_'*63)
      print('Terimakasih, semoga sehat selalu😁'+x)
